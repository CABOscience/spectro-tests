...
#### 2092 wd
folder2092 <- paste0(getwd(), '/2092')
dir.create(folder2092)
setwd(folder2092)

# get list of .sig files in the test working directory (Google Drive)
files <- drive_ls(path = '2018-BeauchampRioux-MSc-UdeM/spectra/2018-05-24-JBMCBTests-2092')
...
spectra.df.long <- gather(spectra.df, wvl, refl, `338.1`:`2516.1`) %>%
...
spectra_all <- ggplot(spectra.df.long, aes(x = wvl, y = refl)) +
  ...
  theme_bw()
ggsave(spectra_all, file = 'spectra_all.pdf', height = 60, width = 12, limitsize = F)
...
spectra_plot_from15 <- ggplot(spectra_from15, aes(x = wvl, y = refl.corr)) +
  ...
  xlab('Wavelength (nm)') +
  scale_y_continuous(limits = c(0.85, 1.15))
ggsave(spectra_plot_from15, file = 'spectra_plot_from15.pdf', height = 50, width = 6, limitsize = F)
...
spectra_plot_every5 <- ggplot(spectra_every5, aes(x = wvl, y = refl.corr)) +
  ...
  scale_y_continuous(limits = c(0.85, 1.15)) +
  theme_bw()
ggsave(spectra_plot_every5, file = 'spectra_plot_every5.pdf', height = 60, width = 12, limitsize = F)
